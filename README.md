...
To reproduce the ARC-AGI 2 number: clone the [ARC-AGI-2 corpus](https://github.com/arcprize/ARC-AGI-2) (not vendored), then `python tools/arc_compiler.py <corpus>/data/evaluation data_bin/arc2_eval` and `mojo run -I src src/arc_solve.mojo --report data_bin/arc2_eval/*.task`.
