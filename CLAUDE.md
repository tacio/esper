...
- `src/arc_solve.mojo` — **Held-out generalization driver** (replaced `benchmark.mojo`). Takes `.task` bundle paths via argv (shell-globbed), fits each task's operator on its train pairs via `fit_operator`, scores the **unseen** test pair(s), and reports per-task held-out + train-fit + the train/test gap, then the aggregate solve rate (raises if 0 solved). Uncheatable by memorization.
- `src/arc_compiler.py` — The one sanctioned Python component: offline compiler. `_write_grid`/`_save_grid` (single grid `.bin`) and `_save_task` (a `.task` bundle) are the single source of the on-disk formats; `compile_arc_json` converts ARC JSON. Not part of the runtime path.
