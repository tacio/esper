...
```bash
./esper suite        # = ./run_tests.sh (full suite + format gate)
...
**`./esper`** is the dev harness: one entrypoint that activates the project venv and runs from the repo root, so commands don't need `cd` + `source .venv/bin/activate` + `mojo run -I src` spelled out each time (`run`/`test`/`main`/`solve`/`fmt`/`suite`/`mojo` subcommands; see the script header). Use it for ad-hoc runs.
...
- `src/memory.mojo` — **Phase B seam.** The `Memory` trait: what the ES fits in-context — `param_dim` (replaces `OP_DIM`), `seed`, `fill_scale` (the per-memory ES preconditioner, e.g. the operator's `COLOR_SCALE` group), `apply(weights, inp: Self.Dom.Example, dst)`, with an associated `comptime Dom: Domain` reached as `M.Dom` (Mojo traits can't take parameters, so the domain is an associated type, not `Memory[D]`). `OperatorMemory` wraps the structured operator (zero behaviour change — the suite's green baseline). `MLPMemory` is the **first emergent memory**: a per-cell `1→H→1` tanh MLP (K=1, H=16) whose output layer the ES fits to a colour map (recolor) with **no hand-coded LUT**, output squashed to exactly `[0,9]` (saturation lands on valid extreme colours; the 9→0 wrap fits for free). No runtime memory-selector — the memory is a compile-time choice, each measured on the subset it expresses.
